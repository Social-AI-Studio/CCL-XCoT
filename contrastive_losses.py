...
class ContraCLMSeqLoss(nn.Module):
    ...
    def forward(self, last_hidden_states_1, last_hidden_states_2, token_mask1, token_mask2):
        device = last_hidden_states_1.device
        batch_size = last_hidden_states_1.size(0)

        # get the sequence representation via mean pooling
        token_mask1 = token_mask1.unsqueeze(-1)
        token_mask2 = token_mask2.unsqueeze(-1)
        features_1 = torch.sum(last_hidden_states_1 * token_mask1, dim=1) / torch.sum(token_mask1, dim=1)
        features_2 = torch.sum(last_hidden_states_2 * token_mask2, dim=1) / torch.sum(token_mask2, dim=1)
        features_1, features_2 = F.normalize(features_1, dim=1), F.normalize(features_2, dim=1)
        features = torch.cat([features_1, features_2], dim=0)
